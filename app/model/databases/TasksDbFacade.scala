  ...
  def addNewTask(task: Task): Option[Long] =
    db.withConnection { implicit connection =>
      SQL("insert into tasks(title , description , notes, is_category," +
          ...
          "values ({title} , {description} , {notes}, {is_category}, {due_date}, {user_created_id}, {project_id}, {business_id}," +
          " {modified_date}, {created_date})")
      .on("title"  -> task.title, "description" -> task.description, "notes" -> task.notes,
            "is_category" -> task.is_category,
            "due_date" -> task.due_date, "user_created_id" -> task.user_created_id, "project_id" -> task.project_id,
